...
# =============================================================================
# Upload AppImage + sync install script to narzaut/rattin-public
...
gh release upload v1.0.0-native "$APPIMAGE" \
    --repo narzaut/rattin-public --clobber

echo "Syncing install script to narzaut/rattin-public..."
INSTALL_REPO="/tmp/rattin-public-sync"
rm -rf "$INSTALL_REPO"
gh repo clone narzaut/rattin-public "$INSTALL_REPO" -- --depth 1
...
if ! git diff --quiet; then
    ...
    git commit -m "sync install script from narzaut/rattin-public"
    git push
fi
rm -rf "$INSTALL_REPO"

echo "Done. Users can now install with:"
echo "  curl -fsSL https://raw.githubusercontent.com/narzaut/rattin-public/main/install-native.sh | bash"
